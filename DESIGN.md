...
- Fixed P1: mobile navigation recovery remains visible below 600px through bottom tabs, and the landing hero no longer lets primary CTAs sit under that fixed nav. `docs/evidence/customcard-mobile.png` now shows the current bottom-nav pattern.
- Fixed P1: current app screenshots in `docs/evidence/customcard-*.png` were recaptured from the production landing, Studio template-review, Print handoff, My cards, and admin-gate screens. They no longer show the older sidebar/grid UI.
