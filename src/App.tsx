...
export function AdminPanelView({
  ...
  const [providerQueueStatus, setProviderQueueStatus] = useState<ProviderQueueStatusState>({ status: "loading" });
  const refreshProviderQueueStatus = useCallback(async () => {
    setProviderQueueStatus((current) =>
      current.status === "loading" ? current : { ...current, refreshing: true }
    );
    setProviderQueueStatus(await fetchProviderQueueStatus(getAdminApiToken));
  }, [getAdminApiToken]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (active) setProviderQueueStatus(next);
    };
    load();
  ...
  return (
    <section className="adminPanel">
      ...
      <ProviderQueueStatusPanel
        onRefresh={refreshProviderQueueStatus}
        state={providerQueueStatus}
      />
...
function ProviderQueueStatusPanel({
  onRefresh,
  state
}: {
  onRefresh: () => void;
  ...
  return (
    <article className="toolPanel adminWide providerQueueMetrics">
