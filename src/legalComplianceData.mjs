...
export const freeLegalToolOptions = [
    {
        id: "termly-basic",
        label: "Termly Basic",
...
export const legalPolicyLinkDefinitions = [
...
export const legalDocumentLinks = legalPolicyLinkDefinitions.map((definition) => {
...
export const legalComplianceItems = [
    {
...
const legalComplianceRegister = defineReadinessRegister({
    ...
    itemRules(item) {
        const issues = [];
        if (!["eu", "us"].includes(item.region))
            issues.push(`Legal compliance item ${item.id} has unsupported region.`);
        if (!allowedStatuses.has(item.status))
            issues.push(`Legal compliance item ${item.id} has unsupported status.`);
        if (item.freeToolIds.length === 0)
            issues.push(`Legal compliance item ${item.id} must reference at least one free tool.`);
        ...
        if (item.currentEvidence.length === 0)
            issues.push(`Legal compliance item ${item.id} must list current repo-local evidence.`);
        if (item.requiredEvidence.length < 2)
            issues.push(`Legal compliance item ${item.id} must list at least two required evidence items.`);
        if (item.externalReviewRequired !== true)
            issues.push(`Legal compliance item ${item.id} must require external legal review.`);
        if (item.publicClaimAllowed !== false)
            issues.push(`Legal compliance item ${item.id} must keep publicClaimAllowed=false.`);
        if (item.blocksLaunch !== true)
            issues.push(`Legal compliance item ${item.id} must block launch until evidence is attached.`);
        if (!item.blocker)
            issues.push(`Legal compliance item ${item.id} must explain its blocker.`);
        return issues;
    },
    crossRules(_itemsById, items) {
        const issues = [];
        if (items.filter((item) => item.region === "eu").length < 6)
            issues.push("Legal compliance readiness must cover at least 6 EU requirements.");
        if (items.filter((item) => item.region === "us").length < 6)
            issues.push("Legal compliance readiness must cover at least 6 US requirements.");
...
export function summarizeLegalComplianceReadiness(items = legalComplianceItems) {
    return {
        ...legalComplianceRegister.summarize(items),
        blockers: items.filter((item) => item.blocksLaunch).map((item) => item.blocker)
    };
}
export function validateLegalComplianceReadiness(items = legalComplianceItems) {
    return legalComplianceRegister.validate(items);
}
export function buildLegalPolicyLinks(env = {}) {
