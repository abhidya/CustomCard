...
export const providerCatalog: ProviderAdapter[] = [
  ...
  {
    id: "monday-crm-lifecycle",
    label: "Monday CRM contact lifecycle",
    provider: "Monday.com GraphQL API",
    capability: "crm-integration",
    lane: "No-code CRM",
    status: "credential-gated",
    cost: "usage-based",
    credentials: ["MONDAY_API_TOKEN"],
    safetyGates: ["Metadata-only import", "No raw content storage", "Metadata schema validation", "Opt-in only", "Suppression list", "Tenant review", "Revocation handling"],
    roleSurface: ["admin"],
    priority: 39.63,
    detail: "Prepares Monday.com board item searches (contacts/leads) for lifecycle card campaigns. API token auth, 10 req/sec.",
    docsUrl: "https://developer.monday.com/api-reference/docs"
  },
  {
    id: "amazon-seller-customer-lifecycle",
    label: "Amazon seller customer lifecycle",
    provider: "Amazon Selling Partner API (SP-API)",
    capability: "crm-integration",
    lane: "Commerce CRM",
    status: "credential-gated",
    cost: "free-tier",
    credentials: ["AMAZON_SP_CLIENT_ID", "AMAZON_SP_CLIENT_SECRET", "AMAZON_SP_REFRESH_TOKEN", "AMAZON_SP_MARKETPLACE_ID"],
    safetyGates: ["Metadata-only import", "No raw content storage", "LWA OAuth required", "Buyer PII restricted by SP-API policy", "Opt-in only", "Suppression list", "Tenant review"],
    roleSurface: ["admin"],
    priority: 39.64,
    detail: "Prepares Amazon order/buyer metadata for seller-side card campaigns. Requires Amazon Seller Central developer registration and LWA credentials.",
    docsUrl: "https://developer-docs.amazon.com/sp-api/docs/selling-partner-api-developer-guide"
  },
  {
    id: "etsy-seller-customer-lifecycle",
    label: "Etsy seller customer lifecycle",
    ...
    safetyGates: ["OAuth consent required", "Metadata-only import", "No raw buyer PII stored", "Opt-in only", "Suppression list", "Revocation handling"],
    roleSurface: ["admin"],
    priority: 39.65,
    detail: "Imports Etsy shop order and buyer metadata for seller card campaigns. OAuth 2.0, scope: transactions_r.",
  ...
  },
  {
    ...
    provider: "Eventbrite API v3",
    capability: "event-import",
    lane: "Provider integration",
    status: "credential-gated",
    cost: "free-tier",
    credentials: ["EVENTBRITE_CLIENT_ID", "EVENTBRITE_CLIENT_SECRET"],
    safetyGates: ["OAuth consent required", "Event metadata only", "No attendee PII stored", "Revocation handling"],
    roleSurface: ["customer", "admin"],
    priority: 35,
    detail: "Imports events the customer is hosting or attending. 1,000 req/hour per token.",
    docsUrl: "https://www.eventbrite.com/platform/api"
  },
  {
    id: "luma-events",
    label: "Luma calendar events",
    provider: "Luma public API",
    capability: "event-import",
    lane: "Provider integration",
    status: "credential-gated",
    cost: "usage-based",
    credentials: ["LUMA_API_KEY"],
    safetyGates: ["API key scoped to single calendar", "Attendee emails not exposed", "No raw content stored"],
    roleSurface: ["customer", "admin"],
    priority: 36,
    detail: "Lists events managed by the connected Luma calendar. Requires Luma Plus subscription. 200 req/min.",
    docsUrl: "https://docs.luma.com/reference/getting-started-with-your-api"
  },
  {
    id: "meetup-events",
    label: "Meetup upcoming events",
    provider: "Meetup GraphQL API",
    capability: "event-import",
    lane: "Provider integration",
    status: "credential-gated",
    cost: "free-tier",
    credentials: ["MEETUP_CLIENT_ID", "MEETUP_CLIENT_SECRET"],
    safetyGates: ["OAuth consent required", "Event metadata only", "Revocation handling", "500 points/60s rate limit"],
    roleSurface: ["customer", "admin"],
    priority: 37,
    detail: "Fetches events the customer is attending via the Meetup GraphQL API (migrated Feb 2025). Requires Meetup Pro for OAuth app creation.",
    docsUrl: "https://www.meetup.com/graphql/guide/"
  },
  {
    id: "partiful-events",
    label: "Partiful events",
    provider: "Partiful (no official API)",
    capability: "event-import",
    lane: "Manual integration",
    status: "contract-only",
    cost: "manual",
    credentials: [],
    safetyGates: ["Manual export only", "No credentials stored", "No reverse-engineering in production"],
    roleSurface: ["customer"],
    priority: 38,
    detail: "Placeholder for Partiful event import. No official API exists as of 2026. Customer pastes event link or ICS; live integration deferred until Partiful launches a developer program.",
  ...
  },
  {
    ...
    roleSurface: ["admin"],
    priority: 57.55,
    detail: "Local-only ComfyUI REST adapter for one-panel-at-a-time card artwork generation with the CustomCardTextComposer final text workflow.",
