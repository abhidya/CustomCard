...
describe("OAuth provider contracts", () => {
  ...
  it("uses canonical Google OAuth env names for Google providers", () => {
    expect(oauthProviderContracts.find((contract) => contract.provider === "google-calendar")?.requiredEnvVars).toEqual([
      "GOOGLE_OAUTH_CLIENT_ID",
      "GOOGLE_OAUTH_CLIENT_SECRET",
      "GOOGLE_OAUTH_REDIRECT_URI"
    ]);
    expect(oauthProviderContracts.find((contract) => contract.provider === "google-gmail")?.requiredEnvVars).toEqual([
        "GOOGLE_OAUTH_CLIENT_ID",
        "GOOGLE_OAUTH_CLIENT_SECRET"
    ]);
  });
