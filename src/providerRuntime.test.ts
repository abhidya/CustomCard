  ...
  it("builds redacted live-network image request contracts for enabled image providers", () => {
    ...
    expect(buildImageGenerationRuntime("deepai-text2img-image", imageInput, readyEnv, openGates).request).toMatchObject({
      ...
      body: expect.objectContaining({
        text: expect.stringContaining("folded card mockup")
      })
    });
    expect(buildImageGenerationRuntime("deepai-text2img-image", imageInput, readyEnv, openGates).request?.body).not.toMatchObject({
      image_generator_version: expect.anything(),
      negative_prompt: expect.anything(),
