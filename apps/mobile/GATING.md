...
| **Live retail price quotes** | Estimate-only; `retail-printers/operations/start` returns `status: "blocked"`, `liveQuoteEnabled: false`. | Admin safety controls plus provider certification gates server-side. | Provider certification plus admin approval. |
| **Automatic retail orders** | Off. App hands off manually; no order is placed. | Mobile app config embeds disabled order safety, and `realOrdersEnabled: false` remains on every route. | Release owner enables admin safety controls after certification. |
| **Print-shop handoff** | App prepares a manual print package only after proof approval; no hosted checkout form appears. | Disabled order safety plus manual handoff route auth. | Keep as handoff until provider certification and release approval exist. |
