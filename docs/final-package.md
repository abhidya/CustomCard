...
- API boundary: tested `/api/health`, customer/admin/mobile bootstrap,
  ...
  handoff, and data-request mutation handling, and
  ...
  card-project inserts, render-packet inserts, audit insert, and queue-job
  insert.
- Persistence boundary: tested auth-session, account identity, hashed recovery
  challenge, idempotency replay, render-packet repository, import-preview
  repository, card-project repository, queue-job, schema-backed route, demo reset,
  and audit contracts served by
...
| API readiness | `npm run api:doctor` | Passed; 13 routes, 6 idempotent mutation contracts, contract runtime mode, 87 providers, 18 persistence tables, render-packet repository readiness, signed artifact contracts, no live calls or real orders. |
