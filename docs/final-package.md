...
| API readiness | `npm run api:doctor` | Passed; 17 routes, 9 idempotent mutation contracts, contract runtime mode, 121 providers, provider governance, 15 schema-backed routes, server-owned calendar and retail operation start packets, relationship-memory and render-packet repository readiness, signed artifact contracts, no live calls or real orders. |
