...
| API readiness | `npm run api:doctor` | Passed; 18 routes, 10 idempotent mutation contracts, contract runtime mode, 121 providers, provider governance, 16 schema-backed routes, server-owned calendar and retail operation start packets, relationship-memory and render-packet repository readiness, signed artifact contracts, no live calls or real orders. |
