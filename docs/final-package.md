...
- API boundary: tested `/api/health`, customer/admin/mobile bootstrap,
  ...
  card-project inserts, audit insert, and queue-job insert.
- Persistence boundary: tested auth-session, account identity, hashed recovery
  challenge, idempotency replay, import-preview repository, card-project
  repository, queue-job, schema-backed route, demo reset, and audit contracts served by
...
| Tests | `npm run check` | Passed on 2026-06-03: 18 test files, 121 tests. |
...
| API Postgres runtime contract | `npm run api:doctor:postgres` | Passed; fake-pool runtime exercised auth-session lookup, wrong-role blocking, idempotency insert/replay/conflict, repository-backed import-preview insert, repository-backed card-project insert, manual handoff order/consent/event insert, audit insert, and queue-job insert without external DB credentials. |
| API live Postgres integration | `npm run api:doctor:postgres:live` | Passed against an isolated temporary database; migration applied, sessions plus approved memory seeded, real `pg` runtime authorized/blocked sessions, persisted/replayed/conflicted idempotency, wrote one provider connection, imported event, card opportunity, card-project row, manual handoff order/consent/event row, and audit plus queue rows. |
...
| Persistence readiness | `npm run persistence:doctor` | Passed; auth sessions, account identities, account recovery challenges, idempotency replay, import-preview repository readiness, card-project repository readiness, manual vendor handoff order/consent/event readiness, queue jobs, render-packet artifact manifests, artifact-store filesystem/S3-compatible write-read signals, Postgres runtime SQL/doctor/integration signals, append-only audit, demo reset mapping, and 11 schema-backed routes present. |
