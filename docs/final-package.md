...
- API boundary: tested `/api/health`, customer/admin/mobile bootstrap,
  ...
  repository-backed import-preview and card-project mutation handling, and
...
| API Postgres runtime contract | `npm run api:doctor:postgres` | Passed; fake-pool runtime exercised auth-session lookup, wrong-role blocking, idempotency insert/replay/conflict, repository-backed import-preview insert, repository-backed card-project insert, audit insert, and queue-job insert without external DB credentials. |
| API live Postgres integration | `npm run api:doctor:postgres:live` | Passed against an isolated temporary database; migration applied, sessions plus approved memory seeded, real `pg` runtime authorized/blocked sessions, persisted/replayed/conflicted idempotency, wrote one provider connection, imported event, card opportunity, card-project row, and audit plus queue rows. |
