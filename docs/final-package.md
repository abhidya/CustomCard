...
| API readiness | `npm run api:doctor` | Passed; 16 routes, 8 idempotent mutation contracts, contract runtime mode, 121 providers, provider governance, 14 schema-backed routes, server-owned calendar connection start packets, relationship-memory and render-packet repository readiness, signed artifact contracts, no live calls or real orders. |
