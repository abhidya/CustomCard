...
| What was verified? | `docs/verification.md`; latest recorded runs cover unit tests, coverage, build, Chrome smoke, contract API doctor, E2E coverage doctor, external audit readiness doctor, provider governance doctor, capacity plan doctor, printer pricing doctor, localization doctor, memory API doctor, Postgres runtime contract doctor, live Postgres integration doctor, Postgres API HTTP doctor, account-auth doctor, artifact-store doctors including live MinIO/S3-compatible writes, persistence doctor, worker doctor, CI workflow contract, mobile contract tests, and mobile doctor. |
...
customer/admin/API/localization/capacity-profile/E2E-coverage/external-audit-readiness/persistence, process-level Postgres HTTP verification,
