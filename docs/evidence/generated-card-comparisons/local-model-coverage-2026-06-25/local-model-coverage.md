...
- Recommended models installed: 7
- Recommended models locally evaluated: 2
- Recommended models missing: 3
...
| qwen3-8b-or-14b-instruct | missing mid-tier planner between 4B speed and 31B quality | no | no | no | Qwen/Qwen3-8B-GGUF or Qwen/Qwen3-14B-GGUF, Q4_K_M | Pull one mid-tier Qwen if Gemma 31B is too slow for routine benchmark loops. |
| sdxl-base-or-card-checkpoint | production-oriented ComfyUI image comparison baseline | no | no | no | SDXL base 1.0 or a rights-clean SDXL card/stationery checkpoint | Pull if DreamShaper keeps creating faces, fake text, or physical-card artifacts. |
| flux-schnell | higher-quality local ComfyUI image research candidate | no | no | yes | ComfyUI-compatible FLUX.1 Schnell model stack | Optional after SDXL baseline; verify VRAM fit before making it benchmark-critical. |
...
- `qwen3-8b-or-14b-instruct`: Qwen/Qwen3-8B-GGUF or Qwen/Qwen3-14B-GGUF, Q4_K_M. Pull one mid-tier Qwen if Gemma 31B is too slow for routine benchmark loops.
- `sdxl-base-or-card-checkpoint`: SDXL base 1.0 or a rights-clean SDXL card/stationery checkpoint. Pull if DreamShaper keeps creating faces, fake text, or physical-card artifacts.
- `flux-schnell`: ComfyUI-compatible FLUX.1 Schnell model stack. Optional after SDXL baseline; verify VRAM fit before making it benchmark-critical.
