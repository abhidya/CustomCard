...
| R011 | Never place real external orders until certification, live quote, approval, and kill-switch gates pass. | Explicit | Covered and blocked. Review-only public pricing observations and freshness reports are separated from live quotes, and server-owned retail operation start packets return provider portal URLs without preparing provider requests, uploads, payments, or live orders. | `realOrdersEnabled: false`, `REAL_ORDER_KILL_SWITCH=disabled`, `transitionOrder`, `src/printerPricing.ts`, `src/retailPrinterOperationStart.ts`, `scripts/api-server.mjs`, infra env examples, runtime doctor. |
