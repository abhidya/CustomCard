...
| R019 | Keep AI/provider calls deterministic and dry-run testable. | Inferred | Covered by no live AI calls, deterministic extraction/rendering, contract-review fulfillment modes, dry-run provider contracts that never call a network, and provider-runtime local import/export adapters that fail closed on blank source text instead of substituting bundled samples. | `src/freeMvp.ts`, `src/providerRuntime.ts`, `src/agentContracts.ts`, `runOperationalExtraction`, `stableId`, tests for free import, SVG generation, contract-review fulfillment modes, provider dry runs, runtime missing-source blocking, weak-input blocking, and checksums. |
