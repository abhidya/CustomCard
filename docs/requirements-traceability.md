...
| R004 | Support web, iOS, and Android as cross-platform product surfaces. | Explicit | Partially covered. Web is a runnable free MVP with customer/admin panels; mobile is a customer shell/config boundary. | `src/App.tsx`; `apps/mobile/README.md`; `apps/mobile/app.config.js`; `apps/mobile/src/App.tsx`; `tests/infra-contract.test.ts`. |
| R005 | Model provider import for email/calendar with scoped access. | Explicit | Covered as free manual/ICS import, service contracts, and cataloged Gmail/Google Calendar/Microsoft Graph/iCloud adapters; not live OAuth. | `src/freeMvp.ts` `parseFreeImport`; `src/serviceKernel.ts` provider adapters; `src/providerCatalog.ts`; tests in `src/freeMvp.test.ts`, `src/serviceKernel.test.ts`, and `src/providerCatalog.test.ts`. |
...
| R021 | Provide customer and admin panels. | Explicit | Covered in the web UI. Customer panel shows next-card state, local chat, image/render choices, and free fallbacks; admin panel shows provider coverage, env gates, deployment readiness, and blocked vendors. | `src/App.tsx`, `src/styles.css`, `tests/app-smoke.test.ts`. |
| R022 | Load broad service-provider adapters for image generation, integrations, and text chat. | Explicit | Covered as a tested adapter catalog with free local fallbacks and credential-gated external providers; live network calls are not implemented. | `src/providerCatalog.ts`, `src/providerCatalog.test.ts`, `infra/env/.env.example`, `docs/platform-expansion-design.md`. |
| R023 | Include a customer-facing text chat interface. | Explicit | Covered as deterministic local chat transcript in the customer panel and mobile shell; live model providers remain gated. | `buildCustomerChatTranscript`, `CustomerPanelView`, `apps/mobile/src/App.tsx`, `src/providerCatalog.test.ts`, `tests/app-smoke.test.ts`. |
| R024 | Include image-generation/render provider readiness. | Explicit | Covered by free browser SVG renderer and gated OpenAI, Gemini, Stability, Hugging Face, and Replicate adapters; no live paid call. | `src/providerCatalog.ts`, `CustomerPanelView`, `AdminPanelView`, `src/providerCatalog.test.ts`, `tests/app-smoke.test.ts`. |
| R025 | Keep the system cheap and cloud-deployment ready through tested IaC. | Explicit | Partially covered. Free local fallback, droplet compose, Kubernetes manifests, env contract, runtime doctor, and infra tests exist; no real cloud deployment evidence. | `infra/docker-compose.dev.yml`, `infra/docker-compose.droplet.yml`, `infra/k8s/app.yaml`, `infra/env/.env.example`, `tests/infra-contract.test.ts`, `docs/platform-expansion-design.md`. |
...
- Mobile customer shell mirrors the customer panel with card queue, memory, chat, render, and handoff sections.
