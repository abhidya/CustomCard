...
- Account auth doctor applies the committed migration to an isolated database,
  ...
  appends audit.
- Persistence contracts map 15 schema-backed API routes to auth-session,
  ...
  repository, retail operation start audit/idempotency, manual handoff
  order/consent/event, data-request privacy/consent, queue-job, and audit-log
  tables.
...
- Provider catalog covers 124 adapters: 18 ready-local, 91 credential-gated, 9
