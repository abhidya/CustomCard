...
| R026 | Treat test coverage as an explicit quality goal. | Explicit | Covered for core, API, E2E matrix, capacity planning, external audit readiness, localization, persistence, orchestration, and mobile contract modules with V8 coverage thresholds in the standard check command; browser UI remains covered by smoke tests rather than unit coverage instrumentation. | `package.json` `test:coverage`, `check`, and `e2e:coverage:doctor`; `vite.config.ts` coverage thresholds; `src/e2eCoverage.test.ts`; `src/capacityPlan.test.ts`; `src/externalAuditReadiness.test.ts`; `src/apiContracts.test.ts`; `src/localization.test.ts`; `src/persistenceContracts.test.ts`; `src/agentContracts.test.ts`; `tests/mobile-contract.test.ts`; `tests/infra-contract.test.ts`; `docs/verification.md`; `coverage/coverage-summary.json` generated by `npm run check`. |
