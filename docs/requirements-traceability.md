...
| R011 | Never place real external orders until certification, live quote, approval, and admin safety gates pass. | Explicit | Covered and blocked. Review-only public pricing observations and freshness reports are separated from live quotes, and server-owned retail operation start packets return provider portal URLs without preparing provider requests, uploads, payments, or live orders. | `realOrdersEnabled: false`, admin safety controls, `transitionOrder`, `src/printerPricing.ts`, `src/retailPrinterOperationStart.ts`, `scripts/api-server.mjs`, runtime doctor. |
...
| R019 | Keep AI/provider calls deterministic and dry-run testable. | Inferred | Covered by no live AI calls, deterministic extraction/rendering, contract-review fulfillment modes, dry-run provider contracts that never call a network, generic provider-runtime dispatch that fails closed when a declared capability input is omitted, retail Adapter operation plans that fail closed on unknown price/upload/order operations, and provider-runtime local import/export adapters that fail closed on blank source text instead of substituting bundled samples. | `src/freeMvp.ts`, `src/providerRuntime.ts`, `src/retailPrinterAdapters.ts`, `src/agentContracts.ts`, `runOperationalExtraction`, `stableId`, tests for free import, SVG generation, retail Adapter operation-plan failure, contract-review fulfillment modes, provider dry runs, runtime missing-input blocking, runtime missing-source blocking, weak-input blocking, and checksums. |
...
| R021 | Provide customer and admin panels. | Explicit | Covered in the web UI. Customer panel starts with local workspace auth, reviewer/local bootstrap defaults, Paste invite or ICS import, gated Google Calendar readiness, manual Apple/iCloud export readiness, a tested one-prominent-action journey with compact supporting tasks that routes event review before card drafting, sendable deterministic local chat with customer-safe local/transcript copy, card proof path, locale readiness, production safety state, customer-facing print recommendations for the lowest current estimate, fastest pickup candidate, and cheapest shipped option with explicit estimate-only or same-cart coupon proof labels, and price/upload/order print-step tiles that open exact provider portal pages without preparing provider requests; admin panel shows provider coverage, a tested integration-owner workflow for credential vault, hosted Clerk token proof, alert drill, incident runbook, and external audit blockers, localization readiness, dry-run runtime readiness, env gates, CRM/workflow integration readiness, business engagement readiness, production launch gates, external audit readiness, AI provider readiness, observability readiness, retail fulfillment readiness, payment readiness, mobile render readiness, hosted API proof readiness, reviewer DB seed readiness, cloud artifact proof readiness, deployment readiness, capacity profiles, and blocked vendors. | `src/App.tsx`, `src/styles.css`, `src/reviewerBootstrap.ts`, `src/customerWebExperience.ts`, `src/adminOperations.ts`, `src/customerChat.ts`, `tests/app-smoke.test.ts`, `tests/infra-contract.test.ts`, `src/customerWebExperience.test.ts`, `src/adminOperations.test.ts`, `src/customerChat.test.ts`. |
| R022 | Load broad service-provider adapters for image generation, integrations, and text chat. | Explicit | Covered as a tested 130-adapter catalog plus executable no-network dry-run contracts with explicit local adapters, hosted-auth readiness, contact/address-book import readiness, event-platform readiness for Eventbrite/Luma/Meetup plus manual Partiful, CRM lifecycle integration readiness for CSV plus Salesforce/HubSpot/Zoho/Pipedrive/Dynamics/Shopify/Klaviyo/Mailchimp/ActiveCampaign/BigCommerce/WooCommerce/Square/Intercom/Monday.com/Amazon Seller/Etsy, business engagement readiness, workflow automation readiness for Zapier/Make/Slack/Teams/Notion/Airtable/Google Sheets/n8n/Workato/Pipedream, notification readiness for email/SMS/WhatsApp/push/customer-messaging providers, payment sandbox readiness for Stripe/PayPal/Square/Adyen, payment readiness for 4 sandbox payment provider contracts and 1 no-payment fallback, AI provider readiness for 16 text and 18 image provider contracts, sendable local customer-chat session backed by the deterministic adapter, observability readiness for error/product/trace/metric/log providers, retail fulfillment readiness for 6 blocked retail-printer contracts and 2 manual fallbacks, explicit Walmart/FedEx/CVS/Walgreens retail adapter contracts for exact product URLs, price fetch, image upload, order placement, blocked request blueprints, server-owned `/api/retail-printers/operations/start` packets, and 12 certification packets for future certified transports, public printer pricing research with 12 official-source observations across 8 persisted source links, coupon provider/feed plus retailer coupon-page collection, provider-portal coupon-application proof policy, and freshness rules, local print package export, filesystem, injected S3-compatible, and live MinIO/S3-compatible artifact-store write/read verification, 98 credential-gated external providers, and hard-blocked live vendors. Generic provider-runtime adapters stay no-network and fail closed by default; the same-origin AI routes can execute configured text/image providers only when server-owned credentials, Admin Providers policy, auth, and cost gates allow them. Live vendor, payment, workflow, customer-message, and retail-order mutations remain blocked or unimplemented. Generic provider runtime dispatch blocks omitted declared inputs instead of fabricating request/local-result data from bundled samples; local import and workflow-export adapters also block missing source text. | `src/providerCatalog.ts`, `src/providerRuntime.ts`, `src/retailPrinterRegistryData.mjs`, `src/retailPrinterContracts.ts`, `src/retailPrinterAdapters.ts`, `src/retailPrinterOperationStart.ts`, `src/customerChat.ts`, `src/businessEngagementReadiness.ts`, `src/businessEngagementReadinessData.mjs`, `scripts/business-engagement-readiness-doctor.mjs`, `src/aiProviderReadiness.ts`, `src/aiProviderReadinessData.mjs`, `scripts/ai-provider-readiness-doctor.mjs`, `src/observabilityReadiness.ts`, `src/observabilityReadinessData.mjs`, `scripts/observability-readiness-doctor.mjs`, `src/retailFulfillmentReadiness.ts`, `src/retailFulfillmentReadinessData.mjs`, `scripts/retail-fulfillment-readiness-doctor.mjs`, `src/paymentReadiness.ts`, `src/paymentReadinessData.mjs`, `scripts/payment-readiness-doctor.mjs`, `src/printerPricing.ts`, `scripts/printer-pricing-doctor.mjs`, `src/printExport.ts`, `src/artifactStore.ts`, `scripts/artifact-store-s3-live-doctor.mjs`, `src/providerCatalog.test.ts`, `src/providerRuntime.test.ts`, `src/retailPrinterAdapters.test.ts`, `src/retailPrinterOperationStart.test.ts`, `src/customerChat.test.ts`, `src/businessEngagementReadiness.test.ts`, `src/aiProviderReadiness.test.ts`, `src/observabilityReadiness.test.ts`, `src/retailFulfillmentReadiness.test.ts`, `src/paymentReadiness.test.ts`, `src/printerPricing.test.ts`, `src/printExport.test.ts`, `src/artifactStore.test.ts`, `infra/env/.env.example`, `docs/platform-expansion-design.md`, `docs/printer-pricing-research.md`. |
...
| R024 | Include image-generation/render provider readiness. | Explicit | Covered by free browser SVG renderer plus dry-run gated OpenAI, Azure OpenAI, Amazon Bedrock, Gemini, Cloudflare Workers AI, Stability, Hugging Face, DeepAI, Replicate, Together, Ideogram, Leonardo, fal, Black Forest Labs, RunComfy Model API, Adobe Firefly, Recraft, and Luma request contracts. Source defaults route card copy through Cloudflare Workers AI `@cf/qwen/qwen3-30b-a3b-fp8`, route card images through `local-comfyui-api-image` with the Flux2 Klein production-text workflow, and keep `cloudflare-workers-ai-image` available as a hosted fallback using `@cf/black-forest-labs/flux-1-schnell`; optional split Cloudflare tokens can separate text and image lanes. Image Runtime contracts use `folded-card-four-panel-v1`: one no-network provider request per front cover, back cover, inside-left, and inside-right panel, with PII redaction and metadata on every panel request so providers do not receive a vague four-up collage prompt. `npm run ai:doctor` tracks model allowlist, prompt audit, image print QA, spend, evaluation, and rollout evidence; no live paid call is claimed by the doctor. | `src/providerCatalog.ts`, `src/providerRuntime.ts`, `src/aiFlowConfigData.mjs`, `src/aiProviderReadiness.ts`, `scripts/ai-provider-readiness-doctor.mjs`, `docs/cloudflare-workers-ai-setup.md`, `CustomerPanelView`, `AdminPanelView`, `src/providerCatalog.test.ts`, `src/providerRuntime.test.ts`, `src/aiProviderReadiness.test.ts`, `tests/app-smoke.test.ts`. |
...
| R030 | Define user stories, onboarding processes, and popular calendar integration planning. | Explicit | Covered as typed onboarding stories, ordered onboarding stages, customer-visible onboarding choices, server-owned `/api/calendar/connections/start` packets, Google Calendar/iCloud readiness contracts, and server-side `/api/import-preview` parsing for explicit metadata-only payloads or pasted raw invite/ICS text. Google Calendar is credential-gated behind OAuth setup and `calendar.events.readonly` / `https://www.googleapis.com/auth/calendar.events.readonly`; when the server has `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, and `GOOGLE_OAUTH_REDIRECT_URI`, the API can return a server-generated Google consent URL, while callback handling, token exchange, credential persistence, revocation, background import, and card creation remain separate blocked gates. iCloud is manual ICS export contract-only. Web and mobile surfaces render the server-owned start packet state and present `Paste invite or ICS` as the ready path; clients do not prepare provider requests. Raw pasted calendar text is parsed into metadata-only preview fields and is not persisted or echoed. No fake live OAuth, Apple credential storage, background sync, card project creation, memory creation, vendor sharing, payment, or order is enabled by these contracts. | `src/onboardingCalendar.ts`; `src/apiContracts.ts`; `src/persistenceContracts.ts`; `src/importPreviewMetadata.mjs`; `src/importPreviewMetadata.test.ts`; `src/onboardingCalendar.test.ts`; `src/apiContracts.test.ts`; `src/persistenceContracts.test.ts`; `src/App.tsx`; `apps/mobile/src/customerExperience.ts`; `apps/mobile/src/App.tsx`; `tests/app-smoke.test.ts`; `tests/api-server.test.ts`; `tests/mobile-contract.test.ts`; `docs/onboarding-calendar-plan.md`; `docs/product-brief.md`; `docs/implementation-roadmap.md`. |
...
- Account auth doctor applies the committed migration to an isolated database,
  ...
  appends audit.
- Persistence contracts map 20 table contracts, 26 stateful API routes, and 14
  idempotent mutations to auth-session, account identity, account recovery,
  idempotency, relationship-memory repository, render-packet repository,
  import-preview repository, card-project repository, retail operation start
  audit/idempotency, manual handoff order/consent/event, data-request
  privacy/consent, provider usage ledger, queue-job, and audit-log tables.
...
- Mobile customer app keeps the native shell customer-facing with a tested
  ...
  Ready customer actions render as accessible native buttons; print and checkout
  actions stay disabled until proof approval, while proof/readiness diagnostics
  stay in contract data, doctors, and admin/web proof surfaces.
...
- Provider catalog covers 130 adapters: 16 ready-local, 98 credential-gated, 10
...
- Admin/adapters UI surfaces no-network runtime readiness, blocked dry-run
  ...
  for credential vault, hosted Clerk token proof, alert route drill, and incident
  review evidence.
...
- No completed OAuth callback, token exchange, credential persistence,
  revocation, or live import flow.
- No deployed production Postgres API integration or production hosted
  Clerk JWT verification outside the isolated live Postgres route-auth,
