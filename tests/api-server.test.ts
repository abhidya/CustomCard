...
describe("api server wrapper", () => {
  it("passes its doctor contract", () => {
    ...
    expect(report.readiness.providers.total).toBeGreaterThanOrEqual(75);
  ...
  it("serves API readiness, bootstrap, and contract-only mutation responses", async () => {
    ...
    try {
      ...
      expect(readiness.providers).toMatchObject({ total: 75, readyLocal: 14, credentialGated: 46, blocked: 6 });
