...
function checkNoBlockers(lane, id, blockers) {
  return {
    ...
    detail: blockers.length === 0 ? "Executable observability readiness contract has no blockers." : blockers.join(" ")
  };
}
...
function checkIncludes(lane, id, text, required) {
  ...
  return {
    ...
    detail:
      missing.length === 0
        ? `Found ${required.length} required observability signals.`
        : `Missing observability signals: ${missing.join(", ")}`
  };
}
