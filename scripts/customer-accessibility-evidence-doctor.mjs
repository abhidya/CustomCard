...
function checkNoBlockers(lane, id, blockers) {
  return {
    ...
    detail: blockers.length === 0 ? "Executable customer accessibility evidence contract has no blockers." : blockers.join(" ")
  };
}
...
function checkIncludes(lane, id, text, required) {
  ...
  return {
    ...
    detail:
      missing.length === 0
        ? `Found ${required.length} required customer accessibility signals.`
        : `Missing customer accessibility signals: ${missing.join(", ")}`
  };
}
...
function checkArrayIncludes(lane, id, values, required) {
  ...
  return {
    ...
    detail:
      missing.length === 0
        ? `Found ${required.length} required customer accessibility evidence signals.`
        : `Missing customer accessibility evidence signals: ${missing.join(", ")}`
  };
}
