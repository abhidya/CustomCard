...
function checkNoBlockers(lane, id, blockers) {
  return {
    ...
    detail: blockers.length === 0 ? "Executable cloud artifact proof readiness contract has no blockers." : blockers.join("; ")
  };
}
...
function checkArrayIncludes(lane, id, values, required) {
  ...
  return {
    ...
    detail:
      missing.length === 0
        ? `Found ${required.length} required cloud artifact proof readiness signals.`
        : `Missing cloud artifact proof readiness signals: ${missing.join(", ")}`
  };
}
