...
- API contract/server boundary with `/api/health`, customer/admin bootstrap,
  ...
  handoff, and data-request mutation coverage, admin demo reset, and no live
  external calls.
- Persistence contract/migration boundary for auth sessions, hosted account
  ...
  event/opportunity writes, card-project repository writes, render-packet
  repository writes, manual handoff order/consent/event writes, data-request
  privacy/consent writes, queue jobs, audit logs, and 11 schema-backed API
  routes.
